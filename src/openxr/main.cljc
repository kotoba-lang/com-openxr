(ns openxr.main
  "Kotodama WASM entrypoint for the Openxr clean-room actor (L4) — Clojure port.
  ...
  facts (`openxr.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "openxr")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instance"          :plural "instances"          :id-prefix "openxr_ins"
    :fields [:applicationName :applicationVersion :engineName :engineVersion :apiVersion :enabledExtensionCount]
    :required [:applicationName :applicationVersion]
    :coerce {:applicationVersion :int :engineVersion :int :apiVersion :int :enabledExtensionCount :int}
    :refs {}}
   {:entity "System"            :plural "systems"            :id-prefix "openxr_sys"
    :fields [:systemId :formFactor :systemName :vendorId :maxSwapchainImageWidth :maxSwapchainImageHeight :orientationTracking :positionTracking]
    :required [:formFactor :systemName]
    :coerce {:systemId :int :formFactor :int :vendorId :int :maxSwapchainImageWidth :int :maxSwapchainImageHeight :int :orientationTracking :bool :positionTracking :bool}
    :refs {:systemId "System"}}
   {:entity "Session"           :plural "sessions"           :id-prefix "openxr_ses"
    :fields [:systemId :createFlags :state]
    :required [:createFlags :state]
    :coerce {:systemId :int :createFlags :int :state :int}
    :refs {:systemId "System"}}
   {:entity "ViewConfiguration" :plural "viewconfigurations" :id-prefix "openxr_vie"
    :fields [:viewConfigurationType :recommendedImageRectWidth :recommendedImageRectHeight :recommendedSwapchainSampleCount :environmentBlendMode]
    :required [:viewConfigurationType :recommendedImageRectWidth]
    :coerce {:viewConfigurationType :int :recommendedImageRectWidth :int :recommendedImageRectHeight :int :recommendedSwapchainSampleCount :int :environmentBlendMode :int}
    :refs {}}
   {:entity "Swapchain"         :plural "swapchains"         :id-prefix "openxr_swa"
    :fields [:createFlags :usageFlags :format :sampleCount :width :height :faceCount :arraySize :mipCount]
    :required [:createFlags :usageFlags]
    :coerce {:createFlags :int :usageFlags :int :format :int :sampleCount :int :width :int :height :int :faceCount :int :arraySize :int :mipCount :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"openxr.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "openxr-compat" :tier tier :entities entities} 200])
...
(defn start! [] :openxr-compat/ready)
